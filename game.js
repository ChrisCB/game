...
var letterSelection = [];           // empy selection array, gets defined in function
var i = 0;                          // Iterator for loop to select letters
var timeAllowed = 106;              // How long the player has (controls )
...
const formButton = document.querySelector('.pickLetterButton');             // The "Draw letters buttom"
const resultsDiv = document.getElementById('results');                      // The box you drag in to 
const changeLetterLink = document.getElementById('redrawLink');             // The link to get new letters


// Pick an individual letter. Probablity based on frequency of letter in English.
function pickLetter(){
    let alphabetPos = Math.ceil(Math.random() * 100)
    let letter = "E";
    switch(true) {
        case (alphabetPos < 8):
            letter = "A"
...
function newLetters(){
    ...
    pickLettersDiv.innerHTML = ``;
    letterSelection.length = 0
    pickLetters();
    //preventDefault();
    console.log(`new letters please!`);
...
pickLetterButton.addEventListener('click', startGame); // Button that starts the game (adds letters and starts timer)

changeLetterLink.addEventListener('click', newLetters); // Linkd that adds letters but doens't start timer
